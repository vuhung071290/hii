...
public class APIService extends BaseService {
  ...
  public Future<User> registerUser(String jsonData) {
    ...
    if (StringUtils.isBlank(user.getUserName())) {
      future.fail(
          new HiiHttpStatusException(
              HttpStatus.BAD_REQUEST.code(),
              ErrorCode.REGISTER_USERNAME_EMPTY.code(),
    ...
    if (StringUtils.isBlank(user.getFullName())) {
      future.fail(
          new HiiHttpStatusException(
              HttpStatus.BAD_REQUEST.code(),
              ErrorCode.REGISTER_FULLNAME_EMPTY.code(),
    ...
    if (StringUtils.isBlank(user.getPassword())) {
      future.fail(
          new HiiHttpStatusException(
              HttpStatus.BAD_REQUEST.code(),
              ErrorCode.REGISTER_PASSWORD_EMPTY.code(),
  ...
  public Future<ChatListResponse> getChatList(String userId) {
    ...
    getChatListsFuture.compose(
        chatLists -> {
          List<Future> getUnSeenCountFutures = new ArrayList<>();
          for (ChatList chatList : chatLists) {
              log.info(userId + ":" + chatList.getSessionId());
            getUnSeenCountFutures.add(
                dataRepository.getUnseenCount(userId, chatList.getSessionId()));
          }
              ...
              ar -> {
                if (ar.succeeded()) {
                  ...
                  for (int index = 0; index < getUnSeenCountFutures.size(); ++index) {
                    ...
                    List<String> listFullNameExcludedCurrentUser =
                        getListFullNameExcludedCurrentUser(userId, chatList.getUserHashes());
                    if (listFullNameExcludedCurrentUser.size() > 1) {
                      String groupName =
                          listFullNameExcludedCurrentUser.stream()
    ...
    getFriendListsFuture.compose(
        friendLists -> {
          ...
          for (FriendList friendList : friendLists) {
            getUserStatusFutures.add(
                dataRepository.getUserStatus(friendList.getFriendUserHashes().getUserId()));
  ...
  public Future<UsernameExistedResponse> checkUsernameExisted(
      UsernameExistedRequest usernameExistedRequest, String userId) {

    Future<UsernameExistedResponse> future = Future.future();

    Future<UserAuth> getUserAuthFuture =
        dataRepository.getUserAuth(usernameExistedRequest.getUsername());
    ...
    getUserAuthFuture.compose(
        userAuth -> {
          if (userAuth != null) {
            ...
            isFriendFuture.compose(
                isFriend -> {
                  if (isFriend) {
                    ...
                    throw new HiiHttpStatusException(
                        HttpStatus.BAD_REQUEST.code(),
                        ErrorCode.START_GROUP_CHAT_USERNAME_NOT_FRIEND.code(),
            ...
            throw new HiiHttpStatusException(
                HttpStatus.BAD_REQUEST.code(),
                ErrorCode.START_GROUP_CHAT_USERNAME_NOT_EXISTED.code(),
  ...
  public Future<GetSessionIdResponse> getSessionIdByUserId(
      GetSessionIdRequest getSessionIdRequest, String userId) {
  ...
  public Future<WaitingChatHeaderResponse> waitingChatHeader(
      WaitingChatHeaderRequest waitingChatHeaderRequest, String userId) {

    Future<WaitingChatHeaderResponse> future = Future.future();

    Future<List<UserAuth>> getUserAuthsFuture =
        getUserAuths(Arrays.asList(waitingChatHeaderRequest.getUsernames()));

    getUserAuthsFuture.compose(
        userAuths -> {
          ...
          getUserFullFutures.compose(
              userFulls -> {
                ...
                WaitingChatHeaderResponse waitingChatHeaderResponse =
                    new WaitingChatHeaderResponse();
  ...
  public Future<AddFriendResponse> addFriend(AddFriendRequest addFriendRequest, String userId) {
    ...
    if (StringUtils.isBlank(addFriendRequest.getUsername())) {
      future.fail(
          new HiiHttpStatusException(
              HttpStatus.BAD_REQUEST.code(),
              ErrorCode.ADD_FRIEND_USERNAME_EMPTY.code(),
              "User Name cannot be empty"));
    }
    ...
    getUserAuthFuture.compose(
        userAuth -> {
          if (userAuth != null) {
                ...
                isFriend -> {
                  if (isFriend) {
                    future.fail(
                        new HiiHttpStatusException(
                            HttpStatus.BAD_REQUEST.code(),
                            ErrorCode.ADD_FRIEND_USERNAME_ALREADY.code(),
                    ...
                    getUserFullsFuture.compose(
                        userFulls -> {
                          ...
                          friendList.setCurrentUserHashes(
                              new UserHash(
                                  currentUserFull.getUserId(), currentUserFull.getFullName()));
                          friendList.setFriendUserHashes(
                              new UserHash(
                                  friendUserFull.getUserId(), friendUserFull.getFullName()));

                          Future<FriendList> insertFriendListFuture =
                              dataRepository.insertFriendList(friendList);

                          insertFriendListFuture.compose(
                              friendListRes -> {
                                List<Future> getUserStatusAndUserOnlineFuture = new ArrayList<>();
                                getUserStatusAndUserOnlineFuture.add(
                                    dataRepository.getUserStatus(friendUserFull.getUserId()));
                                getUserStatusAndUserOnlineFuture.add(
                                    isUserOnline(friendUserFull.getUserId()));

                                CompositeFuture cp =
                                    CompositeFuture.all(getUserStatusAndUserOnlineFuture);
                                cp.setHandler(
                                    ar -> {
                                      if (ar.succeeded()) {
            ...
            throw new HiiHttpStatusException(
                HttpStatus.BAD_REQUEST.code(),
                ErrorCode.ADD_FRIEND_USERNAME_NOT_EXISTED.code(),
                "User Name is not existed");
          }
        },
        Future.future()
  ...
  private List<String> getListFullNameExcludedCurrentUser(
      String currentUserId, List<UserHash> userHashes) {
  ...
  public Future<List<FriendList>> getFriendLists(String userId) {
    ...
    if (StringUtils.isBlank(userId)) {
      future.fail(
          new HiiHttpStatusException(HttpStatus.BAD_REQUEST.code(), "", "User Id is empty"));
      return future;
    }
