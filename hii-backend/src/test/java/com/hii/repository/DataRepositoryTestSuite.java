...
public class DataRepositoryTestSuite extends BaseVerticleTestSuite {
  ...
  @Test
  public void test_getKeysByPattern_withChatListKeyPattern_shouldBeListChatListKey(
      TestContext context) {
    ...
    future.compose(
        keysActual -> {
          context.assertTrue(
              keysActual.size() > 0 && keysActual.get(0).contains(getUserIdToTest()));
          async.complete();
        },
        Future.future()
            .setHandler(
  ...
  @Test
  public void test_getKeysByPattern_withChatMessageKeyPattern_shouldBeListChatMessageKey(
      TestContext context) {
    final Async async = context.async();
    ...
    future.compose(
        keysActual -> {
          context.assertTrue(
              keysActual.size() > 0 && keysActual.get(0).contains(getSessionIdToTest()));
          async.complete();
        },
        Future.future()
            .setHandler(
  ...
  @Test
  public void test_getKeysByPattern_withFriendListKeyPattern_shouldBeListFriendListKey(
      TestContext context) {
    final Async async = context.async();
  ...
  public void test_getUserAuth_withExistedUser_shouldBeExistedUserAuth(TestContext context) {
    ...
    future.compose(
        userAuthActual -> {
          context.assertEquals("usera", userAuthActual.getUserName());
          async.complete();
        },
        Future.future()
            .setHandler(
  ...
  public void test_getUserFull_withExistedUser_shouldBeExistedUserFull(TestContext context) {
    ...
    future.compose(
        userFullActual -> {
          context.assertEquals("usera", userFullActual.getUserName());
          async.complete();
        },
        Future.future()
            .setHandler(
  ...
  public void test_insertFriendList_withValidFriendList_shouldSuccess(TestContext context) {
    ...
    friendList.setCurrentUserHashes(
        new UserHash("testInsertFriendListCurrentUserId", "testInsertFriendListCurrentUserName"));
    friendList.setFriendUserHashes(
        new UserHash("testInsertFriendListFriendUserId", "testInsertFriendListFriendUserName"));

    Future<FriendList> future = getDataRepository().insertFriendList(friendList);
    future.compose(
        friendListActual -> {
          context.assertEquals(
              "testInsertFriendListCurrentUserId",
              friendListActual.getCurrentUserHashes().getUserId());
          async.complete();
        },
        Future.future()
            .setHandler(
  ...
  public void test_getFriendList_withExistedFriendList_shouldBeFriendList(TestContext context) {
    ...
    Future<FriendList> future =
        getDataRepository()
            .getFriendList(
                "friend:list:" + getUserIdToTest() + ":" + getAnotherUserIdToTest(),
                getUserIdToTest());
    future.compose(
        friendList -> {
  ...
  public void test_getFriendList_withNotExistFriendList_shouldBeNull(TestContext context) {
    ...
    Future<FriendList> future =
        getDataRepository()
            .getFriendList(
                "friend:list:" + getUserIdToTest() + ":" + GenerationUtils.generateId(),
                getUserIdToTest());
    future.compose(
        friendList -> {
  ...
  public void test_insertChatList_withValidChatList_shouldSuccess(TestContext context) {
    ...
    future.compose(
        chatListActual -> {
          context.assertEquals(
              "testInsertChatListUserId1", chatListActual.getUserHashes().get(0).getUserId());
          async.complete();
        },
        Future.future()
            .setHandler(
  ...
  public void test_insertChatMessage_withValidChatList_shouldSuccess(TestContext context) {
    ...
    chatMessage.setUserHash(
        new UserHash("testInsertChatMessageId", "testInsertChatMessageFullName"));
  ...
  public void test_getChatMessage_withExistedChatMessage_shouldBeChatMessage(TestContext context) {
    ...
    Future<List<String>> getKeysByPatternFuture =
        getDataRepository().getKeysByPattern("chat:message:" + getSessionIdToTest() + "*");
  ...
  public void test_getChatMessage_withNotExistChatMessage_shouldBeNull(TestContext context) {
    ...
    Future<ChatMessage> future =
        getDataRepository()
            .getChatMessage(
                "chat:message:" + getSessionIdToTest() + ":" + GenerationUtils.generateId());
    future.compose(
        chatMessage -> {
  ...
  @Test
  public void test_increaseUnseenCount_withValidUserIdAndSessionId_shouldSuccess(
      TestContext context) {
    final Async async = context.async();

    Future<Long> future =
        getDataRepository()
            .increaseUnseenCount(GenerationUtils.generateId(), GenerationUtils.generateId());
    future.compose(
        unSeenCount -> {
  ...
  @Test
  public void test_getUnseenCount_withUserHaveUnseenMessage_shouldBePositiveNumber(
      TestContext context) {
    final Async async = context.async();

    Future<Long> future =
        getDataRepository().getUnseenCount(getAnotherUserIdToTest(), getSessionIdToTest());
    future.compose(
        unSeenCount -> {
          context.assertTrue(unSeenCount > 0);
          async.complete();
  ...
  public void test_getUnseenCount_withUserNotHaveUnseenMessage_shouldBeZero(TestContext context) {
    ...
    Future<Long> future =
        getDataRepository().getUnseenCount(GenerationUtils.generateId(), getSessionIdToTest());
    future.compose(
        unSeenCount -> {
          context.assertTrue(unSeenCount == 0);
          async.complete();
  ...
  @Test
  public void test_deleteUnseenCount_withValidUserIdAndSessionId_shouldSuccess(
      TestContext context) {
    final Async async = context.async();

    Future<Long> future =
        getDataRepository()
            .deleteUnseenCount(GenerationUtils.generateId(), GenerationUtils.generateId());
  ...
  public void test_deleteFriend_withValidUserIdAndFriendId_shouldSuccess(TestContext context) {
    ...
    Future<Long> future =
        getDataRepository()
            .deleteFriend(GenerationUtils.generateId(), GenerationUtils.generateId());
    checkResultSuccess(context, async, future);
  }
