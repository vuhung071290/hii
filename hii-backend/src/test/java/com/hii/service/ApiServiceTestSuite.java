...
public class ApiServiceTestSuite extends BaseVerticleTestSuite {
  ...
  public void test_checkUsernameExisted_withExistedFriendUser_shouldBeExisted(TestContext context) {
    ...
    usernameExistedRequest.setUsername("userb");
    ...
    usernameExistedResponseExpected.setUsername("userb");

    Future<UsernameExistedResponse> future =
        getApiService().checkUsernameExisted(usernameExistedRequest, getUserIdToTest());
    future.compose(
        usernameExistedResponseActual -> {
          context.assertEquals(
              usernameExistedResponseExpected.getUsername(),
              usernameExistedResponseActual.getUsername());
          async.complete();
        },
        Future.future()
            .setHandler(
  ...
  public void test_checkUsernameExisted_withNotExistedFriendUser_shouldError(TestContext context) {
    final Async async = context.async();
    UsernameExistedRequest usernameExistedRequest = new UsernameExistedRequest();
    usernameExistedRequest.setUsername("userd");
  ...
  public void test_addFriend_withNotExistedFriend_shouldsuccess(TestContext context) {
    ...
    future.compose(
        response -> {
          ...
          getApiService()
              .getDataRepository()
              .deleteFriend(getUserIdToTest(), getYetAnotherUserIdToTest())
              .setHandler(
                  ar -> {
                    if (ar.succeeded()) {
                      context.assertTrue("UserA".equals(name));
                      async.complete();
                    }
                  });
        },
  ...
  public void test_addFriend_withExistedFriend_shouldFailed(TestContext context) {
    ...
    addFriendRequest.setUsername("userb");

    Future<AddFriendResponse> future =
        getApiService().addFriend(addFriendRequest, getUserIdToTest());
    checkResultValidateFailed(context, async, future);
  }
  ...
  public void test_isFriend_withNotFriendUserId_shouldFalse(TestContext context) {
    ...
    Future<Boolean> future =
        getApiService().isFriend(getYetAnotherUserIdToTest(), getAnotherUserIdToTest());
    future.compose(
        response -> {
  ...
  public void test_getUserProfile_withUserId_shouldReturnUserProfile(TestContext context) {
    ...
    future.compose(
        response -> {
          context.assertEquals(response.getUserFullName(), "UserA");
          context.assertEquals(response.getUserName(), "usera");
          async.complete();
        },
        Future.future()
            .setHandler(
    ...
    try {
      future.compose(
          ...
          Future.future()
              .setHandler(
                  handler -> {
                    context.assertTrue(handler.cause() instanceof HiiHttpStatusException);
                    async.complete();
                  }));

    } catch (Exception e) {
      context.assertTrue(e instanceof HiiHttpStatusException);
      async.complete();
    }
  }
}
