...
import com.hii.model.ChatMessage;
import com.hii.model.User;
import com.hii.model.UserHash;
...
public class BaseServiceTestSuite extends BaseVerticleTestSuite {
  ...
  @Test
  public void
      test_insertChatMessagesAndUpdateChatListAndUpdateUnseenCount_withValidChatMessageAndSessionId_shouldSuccess(
          TestContext context) {
    final Async async = context.async();
    ChatMessage chatMessage = new ChatMessage();
    chatMessage.setSessionId(getSessionIdToTest());
    chatMessage.setUserHash(
        new UserHash(getUserFullToTest().getUserId(), getUserFullToTest().getFullName()));
    ...
    Future<ChatMessage> future =
        getApiService().insertChatMessagesAndUpdateChatListAndUpdateUnseenCount(chatMessage);
    future.compose(
        chatMessageActual -> {
          context.assertEquals(
              "test insert chat messages and update chatList and update unseen count",
              chatMessageActual.getMessage());
          async.complete();
        },
        Future.future()
            .setHandler(
  ...
  @Test
  public void test_increaseUnseenCount_withValidSessionIdAndUserIds_shouldSuccess(
      TestContext context) {
    final Async async = context.async();
    ...
    Future<HashMap<String, Long>> future =
        getApiService().increaseUnseenCount(userIds, getSessionIdToTest());
    future.compose(
        userIdToUnseenCountMapActual -> {
